...
class YOLOv5:
    def __init__(self, save_img = False, user_dir = 'YOLOv5_Inferance'):
        self.model, self.device, self.half, self.names, self.colors = self.initilize()
...
if __name__ == '__main__':
    ...
    # time.sleep(60)
    test_path = 'D:\\FYP\\testing_data\\000000528314.jpg'

    data, result_text = yolo.inferance(cv2.imread(test_path))
    # time.sleep(60)
    f = open('exp1-OpenCV.jpg', 'wb')
    f.write(data)
    f.close()
    ...
    data, result_text, _ = yolo.YOLOv5API(img_base64=im_b64)
    # time.sleep(60)
    f = open('exp2-base64.jpg', 'wb')
    f.write(data)
    f.close()

    data, result_text, _ = yolo.YOLOv5API(path=test_path)
    # time.sleep(60)
    f = open('exp3-path.jpg', 'wb')
    f.write(data)
    f.close()